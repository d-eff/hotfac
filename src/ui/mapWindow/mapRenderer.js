...
function updateLoc(_, data) {
  ...
  console.log('offset left', currentZone.offsetLeft, 'top', currentZone.offsetTop);
...
function getCursorPosition(canvas, event) {
  ...
  const offsetX = (Math.abs(currentZone.offsetTop) - parseInt(event.clientX));
  const offsetY = (Math.abs(currentZone.offsetLeft) - parseInt(event.clientY));
...
locCanvas.addEventListener('mousedown', function(e) {
  getCursorPosition(locCanvas, e)
})